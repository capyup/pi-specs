...
- `src/tasks/task-store.ts` - file-backed task store, YAML `TASKS.yaml` parser/renderer, normalization, locking, CRUD, dependencies, deletion, and completed-task cleanup.
...
- `scripts/sync-tasks.mjs` - CLI check/repair path for YAML `TASKS.yaml` files.
...
1. Add `src/tasks/` modules:
   ...
   - `task-store.ts` for in-memory/file-backed storage, YAML `TASKS.yaml` parsing/rendering, locking, CRUD, dependencies, normalization, deletion, and completed-task cleanup.
...
5. Update `spec_scaffold` to create `TASKS.yaml` as readable YAML and to add short convention sentences to `AGENTS.md` when defaults are inferred.
...
7. Add optional `specDir` parameters to task tools so agents can explicitly target the correct `TASKS.yaml` when more than one spec exists.
8. Add `scripts/sync-tasks.mjs` plus package scripts:
   - `npm run tasks:check` verifies every discovered `TASKS.yaml` is normalized.
   - `npm run tasks:repair` rewrites discovered `TASKS.yaml` files into canonical YAML.
...
12. Update README to describe AGENTS-based spec discovery, date-prefixed spec ids, YAML `TASKS.yaml`, task sync/repair scripts, storage modes, `/tasks`, and how task tracking supports spec-driven workflows.
13. Update skills and prompt templates:
   ...
   - `spec-audit`: include `TASKS.yaml` state as workflow evidence.
   - all relevant prompts: when the user steers mid-workflow, update `PRODUCT.md -> TECH.md -> TASKS.yaml -> implementation/tests` as needed.
...
- Behavior #8/#9/#10/#14/#15: unit-test `TaskStore` YAML parse/render/repair behavior with malformed dependency edges.
- Behavior #11/#12/#13: unit-test task CRUD, dependency cleanup, warnings, deletion cleanup, JSON compatibility for non-YAML stores, and file-backed persistence.
- Behavior #16: test prompt/skill text includes steering alignment from PRODUCT to TECH to TASKS.yaml.
...
- Risk: YAML hand edits can still be malformed. Mitigation: keep task entries intentionally compact; PRODUCT/TECH carry detail, and the parser supports explicit id/status/dependency/owner attributes.
- Risk: hand-edited `TASKS.yaml` can drift. Mitigation: `TaskStore` normalizes on write, `tasks:check` detects drift, `tasks:repair` rewrites canonical YAML, and agent instructions require repair before continuing when drift matters.
...
- Consider `TASKS.sqlite` only if future workflows need real relational queries or transactions; do not mix SQLite/JSON semantics into YAML.
